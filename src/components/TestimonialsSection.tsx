        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t) => (
            ...
            >
              ...
              <p className="font-inter text-muted-foreground text-sm leading-relaxed flex-1 mb-6">
                "{t.text}"
              </p>

              <div className="flex items-center gap-3 border-t border-border pt-4">
                <div className="w-10 h-10 rounded-full gradient-amber flex items-center justify-center font-sora font-bold text-amber-foreground text-sm">
                  {t.name[0]}
                </div>
                <div>
                  <div className="font-sora font-semibold text-foreground text-sm">{t.name}</div>
                  <div className="font-inter text-xs text-muted-foreground">{t.location}</div>
                </div>
              </div>
            </div>
          ))}
